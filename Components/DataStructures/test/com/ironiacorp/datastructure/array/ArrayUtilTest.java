...
public class ArrayUtilTest
{
	...
	@Test
	public void testToCollection_String()
	{
		String[] stringArray = {"a", "b", "c", "d", "e"};
		Collection<String> stringCollection = ArrayUtil.toCollection(stringArray);
		assertNotNull(stringCollection);
		assertEquals(stringArray.length, stringCollection.size());
		Iterator<String> i = stringCollection.iterator();
		for (int j = 0; j < stringArray.length; j++) {
			String element = i.next();
			assertEquals(stringArray[j], element);
			assertTrue(stringArray[j] == element);
		}
	}
